...
## Table of Contents
  ...
  * [Advanced monitoring in Kubernetes](#advanced-monitoring-in-kubernetes)
    + [Observability and Prometheus](#observability-and-prometheus)
    + [Architecture](#architecture)
    + [Kubernetes](#kubernetes)
    + [Expose custom metrics](#expose-custom-metrics)
    + [Conclusion](#conclusion)
  * [In need of long-term Prometheus metrics? Thanos will make Marvels!](#in-need-of-long-term-prometheus-metrics-thanos-will-make-marvels)
    + [Prometheus weakness: scalability](#prometheus-weakness-scalability)
...
- On the subject of events, do not forget [BDX I/O 2019](https://www.bdx.io), the 15th of November of 2019.
...
Prometheus stores its metrics on local block storage and shifts blocks every two hours. It can store millions of samples and request across multiple time series at once. [DigitalOcean](https://www.digitalocean.com/) runs Prometheus at scale and ran into a storage issue. No choice: they had to **shard**.
...
In its core, downsampling reduces the number of exposed samples by grouping them and pre-computing _count_, _sum_, _min_, _max_ and _counter_ (rate and rate change) to greatly speed up queries over very long periods.
...
It is even possible to get faster by using a cache layer on top of Thanos called [Trickster](https://github.com/comcast/trickster). The project is in development but offers promising results already, at leased used on top of Prometheus.
...
Finally, for a presentation of Prometheus at scale by Improbable themselves: [Thanos: Global, durable Prometheus monitoring](https://promcon.io/2018-munich/slides/thanos-prometheus-at-scale.pdf).