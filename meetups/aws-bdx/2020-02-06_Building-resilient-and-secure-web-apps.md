...
An **Availability Zone** or _AZ_ is a physically isolated data center. It is connected to the other AZs of the same
Region with a low-latency connection, possesses its own independent power supply and resilience mechanisms.  
...
If you wish to create and visualize CIDR ranges, you may use [this application](https://cidr.xyz/). In VPCs, CIDR for
...
Additional AWS services may interact with your load balancer such as a [Web Application Firewall](https://aws.amazon.com/waf/)
or a [request accelerator](https://aws.amazon.com/waf/) using the global AWS Content Delivery Network. Load balancer
rules can also be configured to routed based on the request data, use weights, etc.
...
The key takeaway is to always plan for failure and have the five pillars of the Well-Architected Framework in mind when designing architectures: Reliability, Security, Cost Optimization, Performance Efficiency and Operational Excellence.